...
{
	auto sqr = []( int value ) {return value * value; };
	const int rad_sq = sqr( radius );
	const auto diam = radius * 2;

	const auto left = x - radius;
	const auto top = y - radius;
	const auto xStart = std::max( -left, 0 ) - radius;
	const auto yStart = std::max( -top, 0 ) - radius;
	const auto xEnd = std::min(  ScreenWidth - left, diam ) - radius;
	const auto yEnd = std::min( ScreenHeight - top, diam ) - radius;

	for( int y_loop = yStart; y_loop < yEnd; y_loop++ )
	{		
		for( int x_loop = xStart; x_loop < xEnd; x_loop++ )
		{
			if( sqr( x_loop ) + sqr( y_loop ) < rad_sq )
			{
				PutPixel( x + x_loop, y + y_loop, c );
			}
		}
	}
}
